...
void LoadImages(std::string &strImageDir, std::vector<std::string> &vstrImageFilenamesRGB,
                std::vector<std::string> &vstrImageFilenamesD, std::vector<std::string> &vstrImageFilenamesS);
...
int main(int argc, char **argv)
{
    ...
    std::vector<std::string> vstrImageFilenamesS;
    std::string strImageDir = std::string(argv[4]);
    LoadImages(strImageDir, vstrImageFilenamesRGB, vstrImageFilenamesD, vstrImageFilenamesS);
    ...
    else if (vstrImageFilenamesS.size() != vstrImageFilenamesRGB.size())
    {
        std::cerr << std::endl << "Different number of images for rgb and semantic." << std::endl;
        return 1;
    }
    ...
    for (int ni = 0; ni < nImages; ni++)
    {
        ...
        imS = cv::imread(vstrImageFilenamesS[ni], cv::IMREAD_UNCHANGED);
        double tframe = ni;
        ...
        {
            std::cerr << std::endl << "Failed to load image at: "
                      << vstrImageFilenamesS[ni] << std::endl;
            return 1;
        }
...
void LoadImages(std::string &strImageDir, std::vector<std::string> &vstrImageFilenamesRGB,
                std::vector<std::string> &vstrImageFilenamesD, std::vector<std::string> &vstrImageFilenamesS)
{
    std::filesystem::path pathColorImageDir(strImageDir + "/rgb");
    for (const auto& imagePath : std::filesystem::directory_iterator(pathColorImageDir))
    {
        vstrImageFilenamesRGB.push_back(imagePath.path().string());
        std::sort(vstrImageFilenamesRGB.begin(), vstrImageFilenamesRGB.end());
    }
    std::filesystem::path pathDepthImageDir(strImageDir + "/depth");
    for (const auto& imagePath : std::filesystem::directory_iterator(pathDepthImageDir))
    {
        vstrImageFilenamesD.push_back(imagePath.path().string());
        std::sort(vstrImageFilenamesD.begin(), vstrImageFilenamesD.end());
    }
    std::filesystem::path pathSemanticImageDir(strImageDir + "/semantic");
    for (const auto& imagePath : std::filesystem::directory_iterator(pathSemanticImageDir))
    {
        vstrImageFilenamesS.push_back(imagePath.path().string());
        std::sort(vstrImageFilenamesS.begin(), vstrImageFilenamesS.end());
    }
}
