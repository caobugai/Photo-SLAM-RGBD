...
void ImGuiViewer::run()
{
    ...
    GLuint SLAM_img_texture, rendered_img_texture, rendered_depth_img_texture, main_img_texture;
    ...
    glGenTextures(1, &rendered_depth_img_texture);
    glBindTexture(GL_TEXTURE_2D, rendered_depth_img_texture);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
    ...
    while(!isStopped() && !glfwWindowShouldClose(window))
    {
        ...
        if (pSLAM_)
        {
            ...
            // render depth image
            // // 渲染深度图
            cv::Mat rendered_depth_img = pGausMapper_->renderDepthFromPose(
                Tcw, rendered_image_width_, rendered_image_height_, true);
            // std::cout<<rendered_depth_img.channels()<<std::endl;
            // rendered_depth_img.convertTo(rendered_depth_img, CV_16U, 6553.5);
            cv::Mat rendered_depth_img_to_show = cv::Mat(rendered_image_height_, padded_sub_image_width_, CV_32FC1, cv::Scalar(0.0f));
            rendered_depth_img.copyTo(rendered_depth_img_to_show(image_rect_sub));
            cv::normalize(rendered_depth_img_to_show, rendered_depth_img_to_show, 0.0, 1.0, cv::NORM_MINMAX);
            cv::cvtColor(rendered_depth_img_to_show, rendered_depth_img_to_show, cv::COLOR_GRAY2RGB);
            // 上传深度图到OpenGL纹理（注意深度图是单通道浮点类型）
            glBindTexture(GL_TEXTURE_2D, rendered_depth_img_texture);
            glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, rendered_depth_img_to_show.cols, rendered_depth_img_to_show.rows,
                        0, GL_RGB, GL_FLOAT, (float*)rendered_depth_img_to_show.data);
            // 使用ImGui创建窗口来显示深度图
            ImGui::SetNextWindowPos(ImVec2(0, SLAM_img_to_show.rows + 40 + rendered_img_to_show.rows  + 40 ), ImGuiCond_Once);
            ImGui::SetNextWindowSize(ImVec2(rendered_image_width_ + 12, rendered_depth_img_to_show.rows + 40), ImGuiCond_Once);
            {
                ImGui::Begin("Current Rendered Depth Frame");
                ImGui::Image((void *)(intptr_t)rendered_depth_img_texture,
                            ImVec2(rendered_depth_img_to_show.cols, rendered_depth_img_to_show.rows));
                ImGui::End();
            }
