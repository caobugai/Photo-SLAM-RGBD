...
def main(rendered_folder_path, real_folder_path, average_metrics_path):
    # 文件夹路径
    ...
    # 打开文件并读取数据
    with open(average_metrics_path, 'r') as file:
        file.readline()
        line = file.readline().strip()  # 读取一行数据并去除两端的空白字符
        numbers = line.split()  # 以空格为分隔符，分割字符串

    # 将字符串转换为浮点数
    ssim, ps, pnsr= map(float, numbers)

    print(pnsr, ssim, average_lpips)
...
if __name__ == "__main__":
    ...
    parser.add_argument("average_metrics_path", type=str, help="Path to the folder containing average_metrics.")

    args = parser.parse_args()
    
    main(args.rendered_folder_path, args.real_folder_path, args.average_metrics_path)
