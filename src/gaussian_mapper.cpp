...
void GaussianMapper::readConfigFromFile(std::filesystem::path cfg_path)
{
    ...
    opt_use_depth_ =
        (settings_file["Optimization.useDepth"].operator int()) != 0;
    ...
    dataset_ = settings_file["Dataset.type"].string();
}
        ...
        if (hasMetInitialMappingConditions()) {
            ...
            std::vector<ORB_SLAM3::MapPoint*> vpMPs;
            {
                ...
                for (const auto& pKF : vpKFs){
                    ...
                    try {
                        ...
                        cv::Mat imgAuximgRGB = pKF->imgLeftRGB;
                        if (this->sensor_type_ == STEREO)
                            imgRGB_undistorted = imgAuximgRGB;
                        else
                            camera.undistortImage(imgAuximgRGB, imgRGB_undistorted);
                        // Auxiliary Image
                        ...
                        // std::cout<<"new_kf->depth_image_.nd = "<<new_kf->depth_image_.dim()<<std::endl;
            ...
            for (auto& kfit : scene_->keyframes()) {
                ...
                if (device_type_ == torch::kCUDA) {
                    ...
                    img_gpu.upload(pkf->img_auxiliary_undist_);
                    ...
                    for (int l = 0; l < num_gaus_pyramid_sub_levels_; ++l) {
                        cv::Mat img_resized;
                        cv::resize(pkf->img_auxiliary_undist_, img_resized,
                                cv::Size(pkf->gaus_pyramid_width_[l], pkf->gaus_pyramid_height_[l]));
                        pkf->gaus_pyramid_depth_image_[l] =
                            tensor_utils::cvMat2TorchTensor_Float32(img_resized, device_type_);
                    }
...
void GaussianMapper::trainForOneIteration()
{
    ...
    torch::Tensor gt_image, mask, gt_depth_image;
    ...
    if (training_level == num_gaus_pyramid_sub_levels_) {
        ...
        gt_depth_image = viewpoint_cam->depth_image_.cuda();
    }
    else {
        ...
        gt_depth_image = viewpoint_cam->gaus_pyramid_depth_image_[training_level].cuda();
    }

    // gt_depth_image = viewpoint_cam->depth_image_.cuda();
    gt_depth_image = gt_depth_image.unsqueeze(0);
    // depth_mask = undistort_mask_[viewpoint_cam->camera_id_];

    // std::cout<<"gt_depth_image.ndimension() = "<<gt_depth_image.ndimension()<<std::endl;
    // std::cout<<"depth_mask.ndimension() = "<<depth_mask.ndimension()<<std::endl;
    ...
    auto rendered_depth_image = std::get<4>(render_pkg);

    // Get rid of black edges caused by undistortion
    torch::Tensor masked_image = rendered_image * mask;

    // torch::Tensor masked_depth_image = rendered_depth_image * depth_mask;

    // std::cout<<"gt_depth_image.sizes() = "<<gt_depth_image.sizes()<<std::endl;
    // std::cout<<"rendered_depth_image.sizes() = "<<rendered_depth_image.sizes()<<std::endl;
    ...
    //             + loss_utils::l1_loss(rendered_depth_image, gt_depth_image);
    torch::Tensor loss;
    if(opt_use_depth_){
        auto Ll1_depth = loss_utils::l1_loss(rendered_depth_image, gt_depth_image);
        loss = (1.0 - lambda_dssim) * Ll1
                + lambda_dssim * (1.0 - loss_utils::ssim(masked_image, gt_image, device_type_)) + Ll1_depth;
    }else{
        loss = (1.0 - lambda_dssim) * Ll1
                + lambda_dssim * (1.0 - loss_utils::ssim(masked_image, gt_image, device_type_));
    }
                ...
                std::string> &kf)
{
    ...
    try {
        ...
        // Image (left if STEREO)
        cv::Mat imgAuximgRGB = std::get<3>(kf);
        if (this->sensor_type_ == STEREO)
            imgRGB_undistorted = imgAuximgRGB;
        else
            camera.undistortImage(imgAuximgRGB, imgRGB_undistorted);
...
void GaussianMapper::recordKeyframeRenderedRGBDepth(
        torch::Tensor &rendered,
        torch::Tensor &ground_truth,
        torch::Tensor &rendered_depth,
        torch::Tensor &ground_truth_depth,
        unsigned long kfid,
        std::filesystem::path result_img_dir,
        std::filesystem::path result_gt_dir,
        std::filesystem::path result_depth_img_dir,
        std::filesystem::path result_depth_gt_dir,
        ...
        std::string name_suffix)
{
    ...
    if (record_rendered_image_) {
        auto image_cv = tensor_utils::torchTensor2CvMat_Float32(rendered_depth);
        image_cv.convertTo(image_cv, CV_16U, 1.0 / pSLAM_->GetDepthMapFactor());
        cv::imwrite(result_depth_img_dir / (std::to_string(getIteration()) + "_" + std::to_string(kfid) + name_suffix + ".png"), image_cv);
    }

    if (record_ground_truth_image_) {
        auto gt_image_cv = tensor_utils::torchTensor2CvMat_Float32(ground_truth_depth);
        gt_image_cv.convertTo(gt_image_cv, CV_16U, 1.0 / pSLAM_->GetDepthMapFactor());
        cv::imwrite(result_depth_gt_dir / (std::to_string(getIteration()) + "_" + std::to_string(kfid) + name_suffix + "_gt.png"), gt_image_cv);
    }
...
{
    if (!initial_mapped_ || getIteration() <= 0)
        return cv::Mat(height, width, CV_32FC1, cv::Vec3f(0.0f));
    std::shared_ptr<GaussianKeyframe> pkf = std::make_shared<GaussianKeyframe>();
    pkf->zfar_ = z_far_;
    pkf->znear_ = z_near_;
    // Pose
    ...
    masked_image = std::get<4>(render_pkg);

    return tensor_utils::torchTensor2CvMat_Float32(masked_image);
}

void GaussianMapper::renderAndRecordKeyframeRGBDepth(
    std::shared_ptr<GaussianKeyframe> pkf,
    float &dssim,
    float &psnr,
    float &psnr_gs,
    ...
    std::string name_suffix)
{
    ...
    auto gt_depth_image = pkf->depth_image_;
    ...
    l1=loss_utils::l1_loss(rendered_depth_image ,gt_depth_image).item().toFloat();
    
    recordKeyframeRenderedRGBDepth(masked_image, gt_image, rendered_depth_image, gt_depth_image, pkf->fid_, result_img_dir, result_gt_dir, result_depth_img_dir, result_depth_gt_dir, result_loss_dir, name_suffix);    
}
    ...
    std::string name_suffix)
{
    ...
    std::filesystem::path depth_image_dir = result_dir / "depth_image";
    if (record_rendered_image_)
        CHECK_DIRECTORY_AND_CREATE_IF_NOT_EXISTS(depth_image_dir);

    std::filesystem::path depth_image_gt_dir = result_dir / "depth_image_gt";
    if (record_ground_truth_image_)
        CHECK_DIRECTORY_AND_CREATE_IF_NOT_EXISTS(depth_image_gt_dir);
    ...
    std::filesystem::path avg_metrics_path = result_dir / "average_metrics.txt";
    std::ofstream out_avg_metrics(avg_metrics_path);
    out_avg_metrics << "##[Gaussian Mapper]average dssim ,average avg_l1, average psnr_gaussian_splatting" << std::endl;

    std::size_t nkfs = scene_->keyframes().size();
    auto kfit = scene_->keyframes().begin();
    float dssim=0.0, psnr=0.0, psnr_gs=0.0, l1=0.0;
    float avg_dssim=0.0, avg_psnr=0.0, avg_psnr_gs=0.0, avg_l1=0.0;
    double render_time=0.0;
    for (std::size_t i = 0; i < nkfs; ++i) {
        renderAndRecordKeyframeRGBDepth((*kfit).second, dssim, psnr, psnr_gs, l1, render_time, image_dir, image_gt_dir, depth_image_dir, depth_image_gt_dir, image_loss_dir);
        avg_dssim += dssim;
        avg_psnr += psnr;
        avg_psnr_gs += psnr_gs;
        avg_l1 += l1;
    ...
    out_avg_metrics << std::fixed << std::setprecision(10) << avg_dssim / nkfs << " " << avg_l1 / nkfs  << " " << avg_psnr_gs / nkfs  <<std::endl;
